...
$_SESSION['WPOA']['PROVIDER'] = 'Scalebridge';
define('HTTP_UTIL', get_option('wpoa_http_util'));
define('CLIENT_ENABLED', get_option('wpoa_scalebridge_api_enabled'));
define('CLIENT_ID', get_option('wpoa_scalebridge_api_id'));
define('CLIENT_SECRET', get_option('wpoa_scalebridge_api_secret'));
define('REDIRECT_URI', rtrim(site_url(), '/') . '/');
define('SCOPE', 'user_email'); // PROVIDER SPECIFIC: 'profile' is the minimum scope required to get the user's id from Google
define('URL_AUTH', "http://localhost:8080/oauth2/authorize?");
define('URL_TOKEN', "http://localhost:8080/oauth2/token?");
define('URL_USER', "http://localhost:8080/api/user/oauth2/me/?");
...
elseif (isset($_GET['code'])) {
	...
	if ($_SESSION['WPOA']['STATE'] == $_GET['state']) {
		...
		// TODO: Synchronize permission sets after login.
		// NOTE: This never gets called because `wpoa_end_login` calls `die()` at the end of that function.
...
function get_oauth_identity($wpoa) {
	...
	switch (strtolower(HTTP_UTIL)) {
		case 'curl':
			...
			$sb_user_info = json_decode($result, false);
...
//			var_error_log($result_obj);
	...
	$oauth_identity['id'] = $result_obj['id'];  //$result_obj['id']; // PROVIDER SPECIFIC: Google returns the user's OAuth identity as id
	$oauth_identity['username'] = $result_obj['customer_id'] . '_' . $result_obj['username'];
//	var_error_log($result_obj);
	...
	$_SESSION['sb_token'] = $_SESSION['WPOA']['ACCESS_TOKEN'];

	$_SESSION['sb_user_info'] = $sb_user_info;
	// Just to make evaluation easier in templates...
	$_SESSION['sb_customer_id'] = $sb_user_info->customer_id;

	$sb_user_roles = $sb_user_info->roles;
	$sb_user_permissions = [];

//	error_log("************************************************************");
//	error_log("************************************************************");
//	var_error_log($sb_user_info);
//	error_log("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&");
//	var_error_log(json_encode($sb_user_info));
//	error_log("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^");
////	var_error_log($sb_user_roles);
//	error_log("************************************************************");
//	error_log("************************************************************");

	foreach($sb_user_roles as $role) {
		if ($role->role_type == 'global') {
			// global role...just use the permission_ids.
			foreach($role->permissions as $permission) {
				$sb_user_permissions[] = $permission;
			}
		} else {
			// component-specific role...flatten out into
			// `<role_type>.<component_id>.<permission_id>`
			foreach($role->permissions as $permission) {
				$permission_key = $role->role_type . '.' . $role->component_id . '.' . $permission;
				$sb_user_permissions[] = $permission_key;
			}
		}
	}

	// Store the aggregated permissions for synchronization once the user has been registered/logged in.
	$_SESSION['sb_user_permissions'] = $sb_user_permissions;

	//// DEBUG: Delete a specific permission from the returned sb_user_permissions array to test logic.
	//$sb_user_permissions = array_diff($sb_user_permissions, array("admin"));

//	// Remove 'sbp.*' capabilities that the user no longer has as a permission...
//	foreach($user->caps as $key => $value) {
//		if (substr($key, 0, 4) === 'sbp.') {
//			$sb_key = substr($key, 4);
//			if (!in_array($sb_key, $sb_user_permissions)) {
//				//error_log('DELETING ' . $key . ' = ' . $value);
//				$user->remove_cap($key);
//			}
//		}
//	}
//
//	// And add any permissions that the user doesn't have a corresponding
//	// 'sbp.*' capability for currently.
//	foreach($sb_user_permissions as $permission) {
//		$permission_key = 'sbp.' . $permission;
//		if (!$user->has_cap($permission_key)) {
//			//error_log('ADDING ' . $permission_key);
//			$user->add_cap($permission_key);
//		}
//	}
...
function sync_user_permissions() {
//	error_log('==========================================================');
//	error_log('==========================================================');
//	error_log('sync_user_permissions');
	// DEBUG: Delete a specific permission from the returned sb_user_permissions array to test logic.
	$sb_user_permissions = $_SESSION['sb_user_permissions'];

//	var_error_log($sb_user_permissions);
//	error_log('==========================================================');

	global $current_user;
	get_currentuserinfo();

//	var_error_log($current_user);
//	error_log('==========================================================');
...
//	$sb_user_permissions = array_diff($sb_user_permissions, array("admin"));
	...
	foreach($sb_user_permissions as $permission) {
		$permission_key = 'sbp.' . $permission;
		if (!$current_user->has_cap($permission_key)) {
			//error_log('ADDING ' . $permission_key);
			$current_user->add_cap($permission_key);
		}
	}
